...
class brca_kd(ExtendedVisionDataset):
        ...
        for slide in dataroot:
            ...
            if self.kd > 0: 
                for feat_fn in feat_list:
                    ...
                    #print ('re: ', re.split('/')[-2:])
                    #fm_feat = f'/home//tcga_brca/UNI_feat_3scale_agg/{re.split("/")[-2]}/{re.split("/")[-1]}'
                    try:
                        fm_folder  = glob.glob(f'/home/Daejeon/dataset/features/tcga_brca/UNI_feat_3scale_regional/{slide_name}*')[0]
                        ...
                        if os.path.exists(fm_feat) == True:
                            ...
                            fm = np.load(fm_feat, allow_pickle=True)
                            match_list.append([feat,fm])
                    except: continue
    ...
    def __getitem__(self, index):
        ...
        if self.kd > 0:
            ...
            seq = self.seq_list[index]#[feat, fm]
    ...
    #                 coor = re.split("/")[-1].split("_")[1]
    #                 #fm_feat2 = f'/home/Daejeon/dataset/features/tcga_brca/conch_agg/{re.split("/")[-2].split(".")[0]}/{re.split("/")[-2].split(".")[0]}_{coor}'
    #                 #print (fm_feat2, os.path.exists(fm_feat2))
    #                 if os.path.exists(fm_feat) == True and os.path.exists(fm_feat2) == True:
    #                     #print (fm_feat2, os.path.exists(fm_feat2), os.path.exists(fm_feat))
    #                     match_list.append([re, fm_feat, fm_feat2])
    #             self.seq_list = match_list
    #         else:
    #             seq_list.append(feat_list)
    #             self.seq_list = np.concatenate (seq_list, axis=0) 
    ...
    #         feat = np.load(self.seq_list[index][0], allow_pickle=True)
    #         fm = np.load(self.seq_list[index][1], allow_pickle=True)
    #         fm2 = np.load(self.seq_list[index][2], allow_pickle=True)
    #         seq = [feat, fm, fm2]
    #         #print ('fm: ', fm.shape, 'feat', feat.shape)
    #     else: 
    #         seq = np.load(self.seq_list[index], allow_pickle=True)
