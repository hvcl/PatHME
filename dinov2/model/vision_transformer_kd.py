...
class PromptedAttentionLayer2(nn.Module):
    ...
    def forward(self, x, y):
        ...
        updated_x = normalized_output[:, :x_proj.shape[1], :]  # Only take x's updated features
...
class LocalizedPromptedAttentionLayer2(nn.Module):
    ...
    def forward(self, x, y):
        ...
        for i in range(x_proj.shape[1]):  # Iterate over each x[i]
            ...
            attn_output = self.dropout(attn_output)
            normalized_output = self.layer_norm(attn_output)
            localized_outputs.append(normalized_output[:, 0:1, :])  # Only take x[i]'s updated feature
...
class HMSA2(nn.Module):
    ...
    def forward(self, x, y):
        ...
        print (f'localized_output before: {x.shape}, after: {localized_output.shape}')

        localized_output2 = localized_output + x
        #print ('localized_output', localized_output.shape)
        # Step 2: Global Attention
        global_output = localized_output2   # Use localized features as input for global HMSA
        ...
        return global_output + localized_output2, localized_output2
...
class LocalizedPromptedAttentionLayer(nn.Module):
    ...
    def forward(self, x, y):
        ...
        for i in range(x_proj.shape[1]):
            ...
            queries = torch.cat([x_proj[:, i:i+1, :], prompt_expanded], dim=1)
            attn_output, _ = self.attn(queries, y_local, y_local)
            attn_output = self.dropout(attn_output)
            normalized_output = self.layer_norm(attn_output)
...
class HMSA(nn.Module):
    ...
    def forward(self, x, y):
        ...
        return global_output#, localized_output
...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        if do_hmsa > 0 :
            print ('buiding HMSA')
            self.hmsa = HMSA(8, output_embed_dim , 8, 5)
    ...
    def forward_features_list(self, x_list, masks_list, L1_list=None):
        ...
        if L1_list != None:
            for x, masks, L1  in zip(all_x, masks_list, L1_list):
                ...
                x_norm = self.norm(x)
                if L1 != None:
                    hmsa_feat = self.hmsa(L1,  x[:, self.num_register_tokens + 1 :])
